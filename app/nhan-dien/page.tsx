...
export default function NhanDienPage() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white p-3.5 sm:p-4 rounded-2xl border border-purple-100 shadow-2xs">
        <div className="relative w-full sm:max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-purple-50/30 border border-purple-200/80 rounded-xl text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-purple-400/20 focus:bg-white transition-all placeholder-slate-400"
          />
        </div>
        <div className="text-xs text-slate-500 font-medium self-end sm:self-center">
          Hiển thị: <strong className="text-purple-600">{filteredCatalog.length}</strong> loại ma túy ngụy trang
        </div>
      </div>
        ...
        {filteredCatalog.map((item) => {
          ...
          return (
            <div
              ...
              className="bg-white border border-purple-100/90 rounded-3xl p-5 sm:p-6 shadow-xs hover:shadow-md transition-all duration-300 hover:-translate-y-1 flex flex-col justify-between space-y-4 group cursor-pointer hover:border-purple-300 relative"
            >
              {/* PHẦN ĐẦU THẺ: TÊN & HUY HIỆU MỨC ĐỘ ĐỘC */}
              <div className="flex items-start justify-between gap-3 border-b border-purple-100/80 pb-3">
                <div>
                  <h2 className="font-extrabold text-base sm:text-lg text-slate-900 group-hover:text-purple-600 transition-colors leading-snug flex items-center gap-1.5">
                    <span>{item.name}</span>
