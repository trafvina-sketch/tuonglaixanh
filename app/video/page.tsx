    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-8 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="space-y-2">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-orange-50 border border-orange-200/80 text-xs font-bold text-orange-700">
        ...
        <div className="flex items-center gap-2 self-start md:self-auto bg-emerald-50 border border-emerald-200 rounded-2xl px-4 py-2.5 text-xs text-emerald-800 font-bold shrink-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-6">
        {videos.map((vid) => {
          ...
          return (
            <div
              key={vid.id}
              className="bg-white border border-slate-200/80 rounded-3xl p-5 sm:p-6 shadow-xs hover:shadow-md transition-all duration-300 hover:-translate-y-1 flex flex-col justify-between space-y-4 group"
            >
              {/* KHUNG NHÚNG VIDEO YOUTUBE BO TRÒN HIỆN ĐẠI */}
              <div className="rounded-2xl overflow-hidden shadow-xs border border-slate-200/60 bg-slate-900">
                ...
                <div>
                  <div className="flex items-center justify-between gap-2 mb-2">
                    ...
                    <span className="text-[11px] text-slate-500 font-medium flex items-center gap-1">
                      <PlayCircle className="w-3.5 h-3.5 text-[#0284c7]" />
                      Trực tuyến YouTube HD
                    </span>
                  </div>

                  <h3 className="font-extrabold text-base sm:text-lg text-slate-900 group-hover:text-[#0284c7] transition-colors leading-snug">
                    {vid.title}
                  </h3>
